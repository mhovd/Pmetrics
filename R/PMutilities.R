...
makeModel <- function(model = "model.txt", data = "data.csv", engine, write = T, silent = F) {
  ...
  fmod$diffeq <- c(  space(5,"SUBROUTINE DIFFEQ(NDIM,T,X,XP,RPAR,IPAR)"),
                     blank(1),
                     space(5,"use npag_utils,only: max_RS_J, k_dvode_reserved"),
                     space(4,"& ,max_ODE_params, max_ODE_comps, i_ig, i_jsub"),
                     space(4,"& ,k_p_start,k_p_end,k_r_start, k_r_end"),
                     space(4,"& ,max_covs, i_dvode_reserved, max_input_dim"),
                     blank(1),
                     space(5,"IMPLICIT REAL*8(A-H,O-Z)"),
                     blank(1),
                     space(5,"double precision, dimension(257) :: rpar"),
                     space(5,"integer, dimension(257) :: ipar"),
                     space(5,"integer JJJ,IRBD"),
                     blank(1),
                     space(5,vardec),
                     blank(1),
                     ...
                     space(5,"DIMENSION X(NDIM),XP(NDIM),P(max_ODE_params)"),
                     space(5,"DIMENSION B(max_ODE_comps),CV(max_covs)"),
                     space(5,"DIMENSION R(max_RS_J),RATEIV(max_input_dim)"),
                     #"!$omp Threadprivate(/PARAMD/,/INPUT/,/DESCR/,/CNST2/)  ",
                     #"!$omp Threadprivate(/INPUT/,/DESCR/,/CNST2/)  ",
                     blank(1),
                     unlist(lapply(diffstate,function(x) space(5,x))),
                     # wmy2017Oct02
                     space(8,"IRBD = i_dvode_reserved"),
                     blank(1),
                     space(8,"AGE = dble(IPAR(IRBD + 1))"),
                     space(8,"ISEX=IPAR(IRBD + 2)"),
                     space(8,"HEIGHT=dble(IPAR(IRBD + 3))"),
                     space(8,"IETHFLAG=IPAR(IRBD + 4)"),
                     space(8,"NDRUG=IPAR(IRBD + 5)"),
                     space(8,"NADD=IPAR(IRBD + 6)"),
                     space(8,"NI=IPAR(IRBD + 7)"), # = 2*NDRUG+NADD
                     space(8,"ND=IPAR(IRBD + 8)"),
                     space(8,"NUMEQT=IPAR(IRBD + 9)"),
                     space(8,"NOBSER=IPAR(IRBD + 10)"),
                     space(8,"M=IPAR(IRBD + 11)"), # = NOBSER
                     space(8,"JSUB=IPAR(i_jsub)"),
                     space(8,"IG=IPAR(i_ig)"),
                     blank(1),
                     # space(8,"write (*,*) X(1), X(2), X(3)"),
                     # blank(1),
                     # wmy2017Sep27 -- /PARAMD/R is all zeroes! So I copied into RPAR and OK now in serial
                     space(8,"JJJ=1"),
                     space(8,"do I = k_r_start,k_r_end"),
                     space(10,"R(JJJ) = RPAR(I)"),
                     space(10,"JJJ=JJJ+1"),
                     space(8,"end do"),
                     # end wmy
                     blank(1),
                     ...
                     space(8,"END DO"),
                     # wmy2017Sep27 -- /PARAMD/P is all zeroes! So I defined RPAR; OK now in serial
                     blank(1),
                     space(8,"JJJ=1"),
                     space(8,"do I = k_p_start,k_p_end"),
                     space(10,"P(JJJ) = RPAR(I)"),
                     space(10,"JJJ=JJJ+1"),
                     space(8,"end do"),
                     # end wmy
                     blank(1),
  ...
  fmod$output <- c(  space(5,"SUBROUTINE OUTPUT(T,Y,X,RPAR,IPAR)"),   
# --------------------------------------------------------
#                    space(5,"IMPLICIT REAL*8(A-H,O-Z)"),                   
#                     space(5,vardec),                  
#                     space(5,"COMMON /PARAMD/ P"),  
...
#                     space(5,"DIMENSION X(20),P(32),Y(MAXNUMEQ),R(max_RS_J),B(20),CV(26)"),
#                     space(5,"double precision, dimension(257) :: rpar"),
#                     space(5,"integer, dimension(257) :: ipar"),
...
#                     blank(1),
# --------------------------------------------------------
# Above replaced w/below on 6/7/2018
# --------------------------------------------------------
                     space(5,"use npag_utils, only: max_RS_J, max_ODE_comps"),
                     space(4,"& ,max_ODE_params, k_dvode_reserved, k_p_start, k_p_end"),
                     space(4,"& ,k_r_start, k_r_end, i_ig, i_jsub, max_covs"),
                     space(4,"& ,i_dvode_reserved, maxnumeq, max_input_dim"),
                     blank(1),
                     space(5,"IMPLICIT REAL*8(A-H,O-Z)"),                   
                     space(5,"double precision, dimension(257) :: rpar"),
                     space(5,"integer, dimension(257) :: ipar"),
                     space(5,"integer JJJ, IRBD"),
                     space(5,vardec),
                     space(5,"DIMENSION X(max_ODE_comps),P(max_ODE_params)"),
                     space(5,"DIMENSION R(max_RS_J),B(max_ODE_comps),CV(max_covs)"),
                     space(5,"DIMENSION Y(maxnumeq),RATEIV(max_input_dim)"),
                     blank(1),
                     unlist(lapply(diffstate,function(x) space(5,x))),
                     blank(1),
                     space(8,"IRBD = i_dvode_reserved"),
...
# --------------------------------------------------------
  ...
  fmod$symbol <- c(  space(5,"SUBROUTINE SYMBOL(NBCOMP)"),
                     space(5,"use npag_utils, only: max_ODE_params"),
                     space(4,"1 , max_input_dim"),
                     space(5,"IMPLICIT REAL*8(A-H,O-Z)"),                   
                     space(5,vardec),                  
                     #space(5,"CHARACTER PSYM(32)*11"),
                     space(5,"CHARACTER PSYM(max_ODE_params)*11"),
                     # space(5,"COMMON /CNST/ N,ND,NI,NUP,NUIC,NP"),   
                     # space(5,"COMMON/BOLUSCOMP/NBCOMP"),  
                     # "!$omp Threadprivate(/CNST/) ",
                     space(5,"integer,DIMENSION(max_input_dim) :: NBCOMP"), 
                     space(5,"integer N,NP"),
                     blank(2),
                     space(8,"DO I = 1,max_input_dim"), # ea. drug can be put into X(NBCOMP(I))       
                     space(10,"NBCOMP(I) = I"),  
                     space(8,"END DO"),
                     blank(1),
  ...
  fmod$getfa <- c(  space(5,"SUBROUTINE GETFA(FA,X,P,R,B,INTLIST)"),
                    space(5,"use npag_utils,only: max_RS_J,max_input_dim"),
                    space(4,"& ,max_ODE_params,max_ODE_comps,max_covs"),
                    space(5,"IMPLICIT REAL*8(A-H,O-Z)"),                   
                    space(5,vardec),   
                    # space(5,"COMMON /PARAMD/ P"),   
                    # space(5,"COMMON /INPUT/ R,B"),  
                    # space(5,"COMMON /DESCR/ AGE,HEIGHT,ISEX,IETHFLG"),      
                    # space(5,"COMMON /CNST2/ NPL,NUMEQT,NDRUG,NADD"), # NPL not be used in NPAG v123
                    # space(5,"COMMON /STATE/ X"),
                    space(5,"DIMENSION R(max_RS_J), FA(max_input_dim)"),
                    space(5,"DIMENSION P(max_ODE_params),CV(max_covs)"),
                    space(5,"DIMENSION X(max_ODE_comps), B(max_ODE_comps)"),
                    space(5,"integer, dimension(128) :: INTLIST"),
                    # "!$omp Threadprivate(/PARAMD/,/INPUT/,/DESCR/,/CNST2/)",
                    blank(2),
                    space(8,"AGE = dble(INTLIST(1))"),
                    space(8,"ISEX=INTLIST(2)"),
                    space(8,"HEIGHT=dble(INTLIST(3))"),
                    space(8,"IETHFLAG=INTLIST(4)"),
                    space(8,"NDRUG=INTLIST(5)"),
                    space(8,"NADD=INTLIST(6)"),
                    space(8,"NI=INTLIST(7)"), # = 2*NDRUG+NADD
                    space(8,"ND=INTLIST(8)"),
                    space(8,"NUMEQT=INTLIST(9)"),
                    space(8,"NOBSER=INTLIST(10)"),
                    space(8,"M=INTLIST(10)"), # = NOBSER
                    blank(1),
                    space(8,"DO I = 1, NADD"),       
                    space(10,"CV(I) = R(2*NDRUG + I)"),  
                    space(8,"END DO"),
  ...
  fmod$getix <- c(  space(5,"SUBROUTINE GETIX(N,X,P,R,B,INTLIST)"),    
                    space(5,"use npag_utils, only: max_RS_J,max_ODE_params"),
                    space(4,"& ,max_covs,max_ODE_comps"),
                    space(5,"IMPLICIT REAL*8(A-H,O-Z)"),                   
                    space(5,vardec),                  
                    # space(5,"COMMON /PARAMD/ P"),   
                    # space(5,"COMMON /INPUT/ R,B"),  
                    # space(5,"COMMON /DESCR/ AGE,HEIGHT,ISEX,IETHFLG"),      
#                    space(5,"COMMON /CNST2/ NPL,NUMEQT,NDRUG,NADD"),
                    # space(5,"DIMENSION P(32),R(max_RS_J),B(20),CV(26),X(20)"),
                    space(5,"DIMENSION R(max_RS_J)"),
                    space(5,"DIMENSION P(max_ODE_params),CV(max_covs)"),
                    space(5,"DIMENSION X(max_ODE_comps), B(max_ODE_comps)"),
                    space(5,"integer, dimension(128) :: INTLIST"),
                    # "!$omp Threadprivate(/PARAMD/,/INPUT/)  ",
                    # "!$omp Threadprivate(/DESCR/,/CNST2/) ",
#                    "!$omp Threadprivate(/CNST2/) ",
                    blank(2),
                    space(8,"AGE = dble(INTLIST(1))"),
                    space(8,"ISEX=INTLIST(2)"),
                    space(8,"HEIGHT=dble(INTLIST(3))"),
  ...
  fmod$gettlag <- c( space(5,"SUBROUTINE GETTLAG(TLAG,X,P,R,B,INTLIST)"),  
                     space(5,"use npag_utils,only: max_RS_J, max_ODE_params"),
                     space(4,"& ,max_covs,max_ODE_comps, max_input_dim"),
                     space(5,"IMPLICIT REAL*8(A-H,O-Z)"),                   
                     space(5,vardec),                  
                     # space(5,"COMMON /PARAMD/ P"),   
                     # space(5,"COMMON /INPUT/ R,B"),  
                     ...
                     # space(5,"COMMON /CNST2/ NPL,NUMEQT,NDRUG,NADD"),
                     # space(5,"COMMON /STATE/ X"),
                     space(5,"DIMENSION P(max_ODE_params),R(max_RS_J)"),
                     space(5,"DIMENSION CV(max_covs),TLAG(max_input_dim)"),
                     space(5,"DIMENSION B(max_ODE_comps), X(max_ODE_comps)"),
                     space(5,"integer, dimension(128) :: INTLIST"),
                     # "!$omp Threadprivate(/PARAMD/,/INPUT/,/DESCR/)  ",
                     # "!$omp Threadprivate(/CNST2/) ",
                     blank(2),
                     space(8,"AGE = dble(INTLIST(1))"),
                     space(8,"ISEX=INTLIST(2)"),
...
# ----------------------------------------------------------
# 6/11/2018 Remove COMMON blocks from ANAL3() and subroutines
#    called by ANAL3().
#
...
#                    space(5,"COMMON /PARAMD/ P"),   
#                    space(5,"COMMON /INPUT/ R,B"),  
## 6/9/18 -- Removed /RATESV/ from SUBROUTINEs CASE
## 6/9/18             space(5,"COMMON /RATESV/ KE,KA,KCP,KPC,V"),      
#                    space(5,"COMMON /CNST2/ NPL,NUMEQT,NDRUG,NADD"),
#                    space(5,"DIMENSION X(20),P(32),R(37),B(20),CV(26)"),
## 6/9/18             "!$omp Threadprivate(/PARAMD/,/INPUT/,/RATESV/,/CNST2/)   ",
#                    "!$omp Threadprivate(/PARAMD/,/INPUT/,/CNST2/)   ",
#                    blank(2),
#                    space(8,"DO I = 1, NADD"),       
#                    space(10,"CV(I) = R(2*NDRUG + I)"),  
#                    space(8,"END DO"),
# --------------------------------------------------------
# Above replaced w/below on 6/11/2018
# --------------------------------------------------------
  
  # 10/6/2018 -- ANAL3() and CASE<N> need to be put in a
  # module.  Also, the anal.mod/o should use npag_utils.
  # The parameters mapping should also be put in a module,
  # that is used by almost everyone.
  fmod$anal3 <- c(  space(5,"SUBROUTINE ANAL3(X,TIN,TOUT,RPAR,IPAR)"), 
                     space(5,"use npag_utils,only: max_RS_J, max_covs"),
                     space(4,"& ,max_ODE_comps, max_ODE_params, max_input_dim"),
                     space(4,"& ,k_r_start, k_r_end, maxnumeq"),
                     space(4,"& ,k_p_start, k_p_end"),
                     space(5,"IMPLICIT REAL*8(A-H,O-Z)"),
                     space(5,"double precision, dimension(257) :: rpar"),
                     space(5,"integer, dimension(257) :: ipar"),
                     space(5,"integer JJJ"),
                     ...
                     unlist(lapply(diffstate,function(x) space(5,x))),
                     blank(1),
                     space(8,"AGE = dble(IPAR(1+23))"),
                     space(8,"ISEX=IPAR(2+23)"),
                     space(8,"HEIGHT=dble(IPAR(3+23))"),
                     space(8,"IETHFLAG=IPAR(4+23)"),
                     space(8,"NDRUG=IPAR(5+23)"),
                     space(8,"NADD=IPAR(6+23)"),
                     space(8,"NI=IPAR(7+23)"), # = 2*NDRUG+NADD
                     space(8,"ND=IPAR(8+23)"),
                     space(8,"NUMEQT=IPAR(9+23)"),
                     space(8,"NOBSER=IPAR(10+23)"),
                     space(8,"M=IPAR(10+23)"), # = NOBSER
                     space(8,"JSUB=IPAR(11+23)"),
                     space(8,"IG=IPAR(12+23)"),
                     blank(1),
                     space(8,"JJJ=1"),
                     space(8,"do I = k_r_start,k_r_end"),
                     space(10,"R(JJJ) = RPAR(I)"),
...
# 6/9/18             "100     CALL CASE1(X(1),T)",
                    "100     CALL CASE1(X(1),T,KE,KA,KCP,KPC,V,RPAR,IPAR)",
                    space(8,"TIN=TOUT"),                 
                    space(8,"RETURN"),
                    "C  CASE 2 SOLUTION - 1 COMP. + 1ST ORDER INPUT",
# 6/9/18             "200     CALL CASE2(X(1),X(2),T)",                   
                    "200     CALL CASE2(X(1),X(2),T,KE,KA,KCP,KPC,V,RPAR,IPAR)",            
                    space(8,"TIN=TOUT"),          
                    space(8,"RETURN"),
                    "C  CASE 3 SOLUTION - 2 COMPARTMENT NO 1ST ORDER INPUT",
# 6/9/18             "300     CALL CASE3(X(1),X(2),T)",                    
                    "300     CALL CASE3(X(1),X(2),T,KE,KA,KCP,KPC,V,RPAR,IPAR)",
                    space(8,"TIN=TOUT"),          
                    space(8,"RETURN"),
                    "C CASE 4 SOLUTION - 2 COMP. + 1ST ORDER INPUT",
# 6/9/18             "400     CALL CASE4(X,T)",   
                    "400     CALL CASE4(X,T,KE,KA,KCP,KPC,V,RPAR,IPAR)",
                    space(8,"TIN=TOUT"),
                    space(8,"RETURN"),            
                    space(8,"END"), 
                    blank(1),
# 6/9/18             space(8,"SUBROUTINE CASE1(X2,T)"),
                    space(8,"SUBROUTINE CASE1(X2,T,KE,KA,KCP,KPC,V,RPAR,IPAR)"),
                    space(8,"use npag_utils,only: max_RS_J,max_ODE_comps"),
                    space(4,"& , k_r_start, k_r_end"),
                    space(8,"IMPLICIT REAL*8(A-H,O-Z)"),                   
                    space(8,"REAL*8 KE,KA,KCP,KPC"),             
                    space(5,"double precision, dimension(257) :: rpar"),
                    space(5,"integer, dimension(257) :: ipar"),
                    space(8,"DIMENSION R(max_RS_J),B(max_ODE_comps)"),                      
# 6/9/18             space(8,"COMMON /RATESV/ KE,KA,KCP,KPC,V"),            
# 6/11/18            space(8,"COMMON /INPUT/ R,B"),     
# 6/9/18             "!$omp Threadprivate(/RATESV/,/INPUT/)  ",
# 6/11/18            "!$omp Threadprivate(/INPUT/)  ",
                    blank(1),
                    space(8,"JJJ=1"),
                    space(8,"do I = k_r_start,k_r_end"),
                    space(10,"R(JJJ) = RPAR(I)"),
                    space(10,"JJJ=JJJ+1"),
                    space(8,"end do"),
                    # space(8,"do I = 1, max_RS_J"),
                    # space(10,"R(I) = RPAR(55 + I)"),
                    # space(8,"end do"),
                    ...
                    blank(1),
#
# 6/9/18             space(8,"SUBROUTINE CASE2(X1,X2,T)"),                  
                    space(8,"SUBROUTINE CASE2(X1,X2,T,KE,KA,KCP,KPC,V,RPAR,IPAR)"),
                    space(8,"use npag_utils,only: max_RS_J,max_ODE_comps"),
                    space(4,"& , k_r_start, k_r_end"),
                    space(8,"IMPLICIT REAL*8(A-H,O-Z)"),                   
                    space(8,"REAL*8 KE,KA,KCP,KPC"),             
...
# 6/11/18            "!$omp Threadprivate(/INPUT/)  ",
                    ...
                    blank(1),
#
# 6/9/18             space(8,"SUBROUTINE CASE3(X2,X3,T)"),                                        
                    space(8,"SUBROUTINE CASE3(X2,X3,T,KE,KA,KCP,KPC,V,RPAR,IPAR)"),
                    space(8,"use npag_utils,only: max_RS_J,max_ODE_comps,"),
                    space(4,"& k_r_start, k_r_end"),
                    space(8,"IMPLICIT REAL*8(A-H,O-Z)"),                   
...
# 6/9/18             space(8,"COMMON /RATESV/ KE,KA,KCP,KPC,V"),            
# 6/11/18            space(8,"COMMON /INPUT/ R,B"),
                    space(8,"DIMENSION EA(2,2),R(max_RS_J),B(max_ODE_comps)"),   
# 6/9/18             "!$omp Threadprivate(/RATESV/,/INPUT/)  ",
# 6/11/18            "!$omp Threadprivate(/INPUT/)  ",
                    # space(8,"do I = 1, max_RS_J"),
                    # space(10,"R(I) = RPAR(55 + I)"),
                    ...
                    blank(1),
#
# 6/9/18             space(8,"SUBROUTINE CASE4(X,T)"),                                             
                    space(8,"SUBROUTINE CASE4(X,T,KE,KA,KCP,KPC,V,RPAR,IPAR)"),
                    space(8,"use npag_utils,only: max_RS_J,max_ODE_comps"),
                    space(4,"& , k_r_start, k_r_end"),
                    space(8,"IMPLICIT REAL*8(A-H,O-Z)"),                   
                    space(8,"REAL*8 KE,KA,KCP,KPC,V,L1,L2"),       
                    space(5,"double precision, dimension(257) :: rpar"),
                    space(5,"integer, dimension(257) :: ipar"),
# 6/9/18             space(8,"COMMON /RATESV/ KE,KA,KCP,KPC,V"),            
# 6/11/18            space(8,"COMMON /INPUT/ R,B"),
                    space(8,"DIMENSION EA(2,2),R(max_RS_J),B(max_ODE_comps),X(max_ODE_comps)"),     
# 6/9/18             "!$omp Threadprivate(/RATESV/,/INPUT/)  ",
# 6/11/18            "!$omp Threadprivate(/INPUT/)  ",
                    # space(8,"do I = 1, max_RS_J"),
                    # space(10,"R(I) = RPAR(55 + I)"),
  ...
  syntaxcheck <- NA
  moduledir <- system.file("code",package="Pmetrics")
  NPAGutils <- paste(moduledir,"/npag_utils.f90",sep="")
  if (length(grep("gfortran", fortran) > 0)) { syntaxcheck <- paste(fortran, "-fsyntax-only", NPAGutils,modelFor) }
  #fortran syntax different for Windows and UNIX
  if (length(grep("ifort", fortran) > 0)) {
    if (OS == 1 | OS == 3) {
      syntaxcheck <- paste(fortran, "-fsyntax-only", NPAGutils, modelFor)
    } else {
      syntaxcheck <- paste(fortran, "/syntax-only", NPAGutils, modelFor)
    }
  }
  # if (length(grep("g95", fortran) > 0)) { syntaxcheck <- paste(fortran, "-fsyntax-only", NPAGutils, modelFor) }
