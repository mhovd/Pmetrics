      USE npag_utils, only : pause, verifyval,max_pop_rand_varbs
     & , i_dvode_reserved, k_dvode_reserved, k_sum_z_sq, k_prod_pr
     & , k_resolve, k_p_start, k_p_end, k_r_start, k_r_end, k_gamma
     & , k_flat, k_sfac, k_ofac, k_c0_base, k_c1_base, k_c2_base
     & , k_c3_base, k_c4_base, k_c5_base, i_errmod, i_is_log10
     & , i_Nnormalobs, i_is_poisson_obs, i_Npoissonobs, i_misval
     & , i_skip_ig, comp_pred_amt, k_rtol, k_atol, i_mf, i_ndim
     & , i_nparams
 ...
 3177     FORMAT(/' NOFIX WAS READ IN FROM THE INSTRUCTION FILE TO '/
     ...
     1              ,C4(IEQ),C5(IEQ)
          RPAR(k_c0_base + IEQ) = C0(IEQ)
          RPAR(k_c1_base + IEQ) = C1(IEQ)
          RPAR(k_c2_base + IEQ) = C2(IEQ)
          RPAR(k_c3_base + IEQ) = C3(IEQ)
          RPAR(k_c4_base + IEQ) = C4(IEQ)
          RPAR(k_c5_base + IEQ) = C5(IEQ)
         END DO
         READ(23,*)
 ...
 7470    CONTINUE
	...
	IF(INOPT .EQ. 0) THEN
        write (*,*)
        write (*,*) "SIMrun or Manual run w/no inx file: INOPT=",INOPT
        write (*,*)
        WRITE(*,5001)
 ...
 9190     CONTINUE
  ...
  914  FORMAT(/' ENTER A POSITIVE VALUE FOR THE TOLERANCE PARAMETERS: ')
   ...
   19 FORMAT(//' FOR EACH OUTPUT EQUATION, ENTER, IN ORDER, '/
     1' C0,C1,C2,C3,C4,C5 WHERE, FOR EACH OBSERVED VALUE, Y, THE ASSAY'/
     3' STANDARD DEVIATION IS SD = C0+C1*Y+C2*Y**2+C3*Y**3):')
	DO IEQ = 1,NUMEQT
	 WRITE(*,1119) IEQ
 1119    FORMAT(/' ENTER THE VALUES FOR [C0,C1,C2,C3,C4,C5] FOR OUTPUT'/
     1'  EQUATION ',I1,': ')
 ...
 8124  FORMAT(' NUMEQT LINES OF ASSAY COEFFICIENTS')
       DO IEQ=1,NUMEQT
        ...
        WRITE(24,2416) (XVERIFY(IXV),IXV=1,6)
	 END DO
       WRITE(24,8126)
  ...
  246   FORMAT(/' THE ASSAY COEFFICIENTS FOR EACH OF THE OUTPUT '/
     1' EQUATIONS, IN ORDER, WERE: ')
        DO IEQ = 1,NUMEQT
         ...
         XVERIFY(6) = C5(IEQ)
         CALL VERIFYVAL(6,XVERIFY)
         WRITE(23,2416) (XVERIFY(IXV),IXV=1,6)
        END DO
       WRITE(23,247)
 ...
 8014   FORMAT(/' THE INFORMATION FROM THE ',I5,' SIMULATED PATIENT'/
  ...
  232   FORMAT(/' THE USER-INPUT INSTRUCTIONS FOR THIS RUN ARE ABOVE.'/
	...
	CALL SIMBIG(PREFIX,EXT,FILEIN,NSUB,NVAR,NOFIX,IRAN,VALFIX,
     1   C0,C1,C2,C3,C4,C5,IDIST,NGAUS,COF,AMEAN,COV,PATH,NDIM,PAR,
  ...
  113    FORMAT(//' YOUR MODEL FILE HAS A TOTAL OF ',I2,' PARAMETERS.'/
	...
	END
     ...
     3   ILIMIT,BOUND,INTLIST,RPAR,IPAR)
        use npag_utils, only : pause, verifyval, makevec, do140
     1  , max_pop_varbs, k_ig, i_ig, comp_pred_amt
     2  , k_sum_z_sq, k_prod_pr, k_sfac, k_ofac, k_resolve, k_gamma
     3  , k_flat, k_c0_base, k_c1_base, k_c2_base, k_c3_base, k_c4_base
     4  , k_c5_base, i_errmod, i_misval, i_Nnormalobs, i_Npoissonobs
     5  , i_is_log10, i_is_Poisson_obs, max_m_per_obs, maxnumeq
        IMPLICIT REAL*8(A-H,O-Z)
        integer, dimension(128) :: INTLIST
        double precision, dimension(257) :: RPAR
        integer, dimension(257) :: IPAR
     ...
     5   VALFIX(20),XVERIFY(900)
         ...
         integer ijob, JSUB, IG, JJ, KK
         dimension F(3564)
         double precision, dimension(594,MAXNUMEQ) :: ObsError, YO
         dimension C4(MAXNUMEQ), C5(MAXNUMEQ)
      ...
      DO I = 1,NVAR
       IF(ILIMIT(I) .EQ. 0) THEN
        IF((ALF(I).LT.BOUND(I,1)).OR.(ALF(I).GT.BOUND(I,2))) then
           IOK = 0
           write (*,*) ISUB,"is outside of restricted bound"
     &       ,I,ALF(I),"; bound=",bound(I,1),bound(I,2)
        ENDIF
       ENDIF
      END DO
 6510  CONTINUE
 ...
 9019    FORMAT(' SUBJECT NO. ',I7)
	...
	OPEN(31,FILE=PATHFIL2,STATUS='OLD')
     ...
     1        C3(J)*YPRED(I,J)**3
        ...
        RPAR(k_flat) = 1.d0
        DO JJ = 1,intlist(9)
          RPAR(k_c0_base + JJ) = C0(JJ)
          RPAR(k_c1_base + JJ) = C1(JJ)
          RPAR(k_c2_base + JJ) = C2(JJ)
          RPAR(k_c3_base + JJ) = C3(JJ)
          RPAR(k_c4_base + JJ) = C4(JJ)
          if (NINT(C4(JJ)).eq.10) then
            IPAR(i_is_log10 + JJ) = 10
          endif
          RPAR(k_c5_base + JJ) = C5(JJ)
          if (NINT(C5(JJ)).eq.229) then
            IPAR(i_is_poisson_obs + JJ) = 229
          endif
        END DO
        IPAR(i_errmod) = 1
        call DO140(ijob, JSUB, IG, INTLIST, IPAR, RPAR
     1   , F, AS, YO, YPRED, ERRFIL)
        DO J=1,NOS
	  DO I=1,M
            IF(Y(I,J) .LE. -99.D0) YPRED(I,J) = -99.D0
	    YPREDSUB(ISUB,I,J) = YPRED(I,J)
...
245       FORMAT(' ',F14.7,21(',',G14.7))
 ...
 1017   FORMAT(2X,I4,' ... NO. OF OBSERVED VALUE TIMES')
	...
	END
     ...
     1  INTLIST,RPAR,IPAR)
      use npag_utils, only :  pause, verifyval, SHIFT
     1  , thesame, predlast3, USERANAL, SimConc
     2  , max_ODE_comps, max_ODE_params, max_SS_doses
     3  , max_RS_J, max_m_per_obs
     4  , k_dvode_reserved, k_p_end, k_jsub, k_ig
     5  , i_dvode_reserved, i_jsub, i_ig, comp_pred_amt
      IMPLICIT REAL*8(A-H,O-Z)
      integer, PARAMETER :: MAXNUMEQ=7
...
2005    Y(KNT,I)=YT(I)
        IF(N .GT. 0) THEN
         DO ICOMPART = 1,N
          comp_pred_amt(KNT,ICOMPART) = X(ICOMPART)
         END DO
        ENDIF
        IF(N .EQ. -1) THEN
         DO ICOMPART = 1,3
...
2010    Y(KNTM1,I)=YT(I)
        IF(N .GT. 0) THEN
         DO ICOMPART = 1,N
          comp_pred_amt(KNTM1,ICOMPART) = X(ICOMPART)
         END DO
        ENDIF
        IF(N .EQ. -1) THEN
         DO ICOMPART = 1,3
...
40      IF(KNT .LE. INTLIST(10)) GO TO 45
	...
	END DO
         ...
         write (*,*) "Calling SimConc()"
         call SimConc(4,JSUB,IG
     1   , intlist(10), F, intlist(9)
     2   , NBCOMP
     3   , NP,P,NDIM,MF,RTOL,ATOL
     4   , TIMN,SIGN,RSN,BSN
	...
	RETURN
	END
        SUBROUTINE Old_USERANAL(X,TIN,TOUT,RPAR,IPAR)
        IMPLICIT REAL*8(A-H,O-Z)
        double precision, dimension(257) :: RPAR
        integer, dimension(257) :: IPAR
 ...
 1102   FORMAT(' ',A78)
  ...
  260 CONTINUE
     ...
     1    INTLIST,RPAR,IPAR)
        use npag_utils, only: pause, verifyval, thesame
	IMPLICIT REAL*8(A-H,O-Z)
	PARAMETER(MAXNUMEQ=7)
        integer, dimension(128) :: INTLIST
        double precision, dimension(257) :: RPAR
  ...
  200 CONTINUE
    ...
    2 FORMAT(/' ONE OF THE LINES IN YOUR .cvs FILE HAS AN ERROR.'/
     ...
     1   ICONV = 0
      ...
      use npag_utils, only: pause,verifyval,orderdelta,thesame
      IMPLICIT REAL*8(A-H,O-Z)
	PARAMETER(MAXNUMEQ=7)
      DIMENSION INTLIST(128)
