...
! Double precision
      ...
      ! note: Both of the following work for non openmp program
        integer, parameter :: dp = kind(1.d0)                   ! Machine-compiler-specific real(dp)
                                                                ! should be equiv. to real*8, or 64 bit
...
!        real(dp) :: RTOL
 Contains
!*DECK DVNORM
      real(kind(1.d0)) FUNCTION DVNORM (N, V, W)
...
!   930809  Renamed to allow single/real(dp) versions. (ACH)
!***END PROLOGUE  DVNORM
!**End
      INTEGER N, I
      real(dp) SUM
      real(dp), DIMENSION(N), intent(in) :: V, W
       ! write (*,*) shape(V)
       ! write (*,*) shape(W)
       ! write (*,*) N
!
...
!*DECK DVODE_REAL
      ...
      interface
        SUBROUTINE DVINDY (T, K, preYH, LDYH, DKY, IFLAG)
           use dvode_globals
          real(dp) T
          INTEGER K, LDYH, IFLAG
          real(dp), DIMENSION( : ), intent(inout) :: preYH
          real(dp), DIMENSION( : ), intent(inout) :: DKY
      ...
      interface
        SUBROUTINE DEWSET (N, ITOL, RTOL, ATOL, YCUR, EWT)
           ...
           real(dp), DIMENSION(:), intent(in)  :: ATOL
           real(dp), dimension(N), intent(inout) :: YCUR, EWT
      ...
      interface
        SUBROUTINE DVHIN (N, T0, Y0, YDOT, F, RPAR, IPAR, TOUT, UROUND  &
          , EWT, ITOL, ATOL, Y, TEMP, H0, NITER, IER)
           use dvode_globals
           EXTERNAL F
           real(dp), intent( inout ) :: H0, T0, TOUT              ! ABS(TOUT - T0)
           real(dp), intent( in ) :: UROUND
           INTEGER, intent( in ) :: ITOL
           integer, intent( inout ) :: N, IER, NITER
           integer, dimension( : ), intent( in ) :: IPAR
           real(dp), dimension( : ), intent(inout) :: Y, TEMP   & ! DVNORM(...N, EWT, TEMP...)
             , RPAR, EWT                                                             ! F(...Y, N, IPAR, RPAR, TEMP ...),
           real(dp), dimension( : ), intent(in) :: Y0, YDOT, ATOL
       ...
       interface
         SUBROUTINE DVSTEP (Y, YH, LDYH, YH1, EWT, SAVF, VSAV, ACOR, WM,   &
           ...
           real(dp) SAVF, ACOR
           DIMENSION SAVF ( * ), ACOR ( * )
           real(dp), dimension (LDYH,*), intent( inout ) :: YH
           real(dp), dimension ( : ), intent ( inout ) :: Y, YH1, EWT, VSAV, WM, RPAR     ! is passed to F
           integer, dimension ( : ), intent ( inout ) :: IWM, IPAR
...
!
      ...
      real(dp), dimension(LRW), intent(INOUT) :: RWORK
      real(dp), dimension(:), intent(INOUT) :: Y, RPAR
      integer,  intent(INOUT) :: ISTATE
      integer,  dimension(:), intent(INOUT) :: IWORK, IPAR
      real(dp), intent(IN) :: RtolIn
      real(dp), dimension(:), intent(IN) :: ATOL
...
!
! RWORK  = A real working array (real(dp)).
...
!         /DVOD01/  of length  81  (48 real(dp) words
!                         followed by 33 integer words),
!         /DVOD02/  of length  9  (1 real(dp) word
...
!            to be unique in combined single/real(dp) code (ACH)
!  19920722  Minor revisions to prologue (ACH).
!  19920831  Conversion to real(dp) done (ACH).
...
!
      real(dp) ACNRM, CCMXJ, CONP, CRATE, DRC, EL, ETA, ETAMAX,  &
...
!
      EXTERNAL DVNLSD
      LOGICAL IHIT
      real(dp) ATOLI, BIG, EWTI, FOUR, H0, HMAX, HMX, HUN, ONE,  &
...
!
      real(dp) DUMACH ! , DVNORM ! dvnorm was added to dvode_globals.mod
!
      DIMENSION MORD (2)
!-----------------------------------------------------------------------
...
! note also that size ( <real(dp) array> ) returns the
!   correct length!!!
...
! the code returns as expected.
      IF (TOUT.EQ.T) then
        write (*,*) JSUB,IG,"is aborting at start of DVODE_v01.f90 w/T=TOUT=",T
        RETURN
      end if
!-----------------------------------------------------------------------
! Block B.
...
! Initial call to F.  (LF0 points to YH(*,2).) -------------------------
      LF0 = LYH + NYH
      ! if (DEBUG == 1) write (*,*) "Calling F for first time"
      CALL F (N, T, Y, RWORK (LF0), RPAR, IPAR)
      ! if (DEBUG.eq.1) then
      !  write (*,*) "Ret. from F for first time"
      ! endif
      NFE = 1
...
! Load and invert the EWT array.  (H is temporarily set to 1.0.) -------
      ...
      CALL DEWSET (N, ITOL, RTOL, ATOL                                  &
        , RWORK (LYH:(LYH+(MAXORD+1)*NYH-1)), RWORK (LEWT:(LEWT+N -1)) )
! ----------------------------------------------------------------------
!      DO 120 I = 1, N
...
!      IER)
!
      CALL DVHIN (N, T, RWORK (LYH:(LYH + N - 1))                       & ! Updating call DVHIN led to 100% failure :-(
        , RWORK (LF0:(LF0 + N - 1)), F, RPAR, IPAR, TOUT, UROUND        &
        , RWORK (LEWT:(LEWT + N  - 1)), ITOL, ATOL, Y                   &
        , RWORK (LACOR:(LACOR + N - 1)), H0, NITER, IER)
      NFE = NFE+NITER
...
!-----------------------------------------------------------------------
  ...
  210 IF ( (TN - TOUT) * H.LT.ZERO) GOTO 250
      CALL DVINDY (TOUT, 0, RWORK (LYH:(LYH+(MAXORD+1)*NYH-1)),  NYH,   Y, IFLAG)
...
!-----------------------------------------------------------------------
  ...
  270 TOLSF = UROUND * DVNORM (N , RWORK (LYH:(LYH+(MAXORD+1)*NYH-1))   &
                                 , RWORK (LEWT:(LEWT+N -1)) )
      ! write (*,*) "Decision: TOLSF, NST, TN, H, NHNIL", TOLSF, NST, TN, H, NHNIL
...
!-----------------------------------------------------------------------
      ! write (*,*) "Calling DVSTEP", LWM, LENWM, shape(RWORK (LWM:(LWM + LENWM - 1)))
      CALL DVSTEP (Y &
       , RWORK(LYH:) & ! RWORK (LYH:(LYH + N - 1)) &
       , NYH &
       , RWORK(LYH:) & !RWORK (LYH:(LYH + N - 1)) &
       , RWORK(LEWT:) & ! RWORK (LEWT:(LEWT + N - 1))      &
       , RWORK(LSAVF:) & ! RWORK (LSAVF:(LSAVF + N - 1)) &
       , Y &
       , RWORK(LACOR:) & ! RWORK (LACOR:(LACOR + N - 1)) &
       , RWORK(LWM:) & ! RWORK (LWM:(LWM + LENWM - 1))  &
       , IWORK       &                      ! LIWM = 1 ; so, why bother sending IWORK(LIWM)?
...
!*DECK DVHIN
      ...
      EXTERNAL F
!
!      real(dp) T0, Y0, YDOT, RPAR, TOUT, UROUND, EWT, ATOL, Y,   &
!      TEMP, H0
...
!
      ...
      integer, intent( inout ) :: N, IER, NITER
      integer, dimension( : ), intent( in ) :: IPAR
      real(dp), dimension( : ), intent(inout) :: Y, TEMP, RPAR  & ! DVNORM(...N, EWT, TEMP...)
        , EWT                                                             ! F(...Y, N, IPAR, RPAR, TEMP ...),
      real(dp), dimension( : ), intent(in) :: Y0, YDOT, ATOL
!
!-----------------------------------------------------------------------
...
! Type declarations for local variables --------------------------------
!
      real(dp) AFI, ATOLI, DELYI, H, HALF, HG, HLB, HNEW, HRAT,  &
...
! Estimate the second derivative as a difference quotient in f. --------
      ...
      DO 60 I = 1, N
         Y (I) = Y0 (I) + H * YDOT (I)
   60 END DO
      ! write (*,*) "Calling F in DVHIN"
      CALL F (N, T1, Y, TEMP, RPAR, IPAR)
      DO 70 I = 1, N
         TEMP (I) = (TEMP (I) - YDOT (I) ) / H
   70 END DO
      ! write (*,*) "In DVHIN: Calling DVNORM"
...
! Iteration done.  Apply bounds, bias factor, and sign.  Then exit. ----
   ...
   90 H0 = SIGN (H0, TOUT - T0)
      NITER = ITER
      IER = 0
      ! write (*,*) "Ret. from DVHIN w/IER=",IER
      RETURN
...
!      real(dp), DIMENSION(:,:), intent(inout) :: YH
      real(dp), DIMENSION( : ), intent(inout) :: preYH ! read preYH into YH
      real(dp), dimension( : ), intent(inout) :: DKY
!-----------------------------------------------------------------------
...
! Type declarations for local variables --------------------------------
!
      real(dp) C, HUN, R, S, TFUZZ, TN1, TP, ZERO
...
!
!-----------------------------------------------------------------------
      real(dp), dimension(2) :: padding
      integer, dimension(2) :: newshape
      real(dp), DIMENSION(LDYH,6) :: YH
         ! 6 = (MAXORD + 1)
         !      MAXORD = 12 (if METH=1) or 5 (if METH=2)
         !      METH = abs(MF)/2; we use MF = 21
      padding = (/ 0.D0, 0.D0 /)
      newshape(1) = LDYH
      newshape(2) = 6
      YH = reshape(preYH,shape=newshape,pad=padding,order=(/1,2/))
         ! reshape(data, shape = (rowsize, colsize, 3dsize, ... ) &
         !    , pad = (/0,0/), order = (,,,))
!-----------------------------------------------------------------------
      IFLAG = 0
...
!*DECK DVSTEP
      ...
      Interface
        SUBROUTINE DVNLSD (Y, YH, LDYH, VSAV, SAVF, EWT, ACOR, IWM, WM, F,&
          JAC, PDUM, NFLAG, RPAR, IPAR)
           use dvode_globals
           EXTERNAL F, JAC, PDUM
           real(dp) YH, VSAV, SAVF, ACOR, EWT
           INTEGER LDYH, NFLAG
           DIMENSION YH (LDYH, * ), VSAV ( * ), SAVF ( * ) , ACOR ( * ), EWT ( * )
         !
          real(dp), dimension(:), intent(inout) :: Y, WM, RPAR
          integer, dimension(:), intent(inout) :: IWM, IPAR
      ...
      real(dp) SAVF, ACOR
      DIMENSION SAVF ( * ), ACOR ( * )
      real(dp), dimension ( : ), intent ( inout ) :: Y, YH1             &
       , VSAV, EWT, WM, RPAR
                    ! preYH is passed to F ; *** BUG *** YH = reshape(YH1) ... not in same space?
      integer, dimension ( : ), intent ( inout ) :: IPAR, IWM
!
!      SUBROUTINE DVSTEP (Y, YH, LDYH, YH1, EWT, SAVF, VSAV, ACOR, WM,   &
!      IWM, F, JAC, PSOL, VNLS, RPAR, IPAR)
!      EXTERNAL F, JAC, PSOL, VNLS
!      real(dp) Y, YH, YH1, EWT, SAVF, VSAV, ACOR, WM, RPAR
...
! Type declarations for local variables --------------------------------
!
      real(dp) ADDON, BIAS1, BIAS2, BIAS3, CNQUOT, DDN, DSM, DUP,&
...
!-----------------------------------------------------------------------
      real(dp), dimension(2) :: padding
      integer, dimension(2) :: newshape
      ! real(dp), DIMENSION(LDYH,6), intent(inout) :: YH
         ! 6 = (MAXORD + 1)
         !      MAXORD = 12 (if METH=1) or 5 (if METH=2)
         !      METH = abs(MF)/2; we use MF = 21
      padding = (/ 0.D0, 0.D0 /)
...
!-----------------------------------------------------------------------
   ...
   50 IF (NEWH.EQ.0) GOTO 200
      ...
      ENDIF
      IF (NEWQ.GT.NQ) THEN
         ! write (*,*) "In DVSTEP: Calling DVJUST"
         CALL DVJUST (YH, LDYH, 1)
         NQ = NEWQ
         L = NQ + 1
         NQWAIT = L
...
!
      ! write (*,*) "In DVSTEP: Calling VNLS", shape(WM)
      CALL DVNLSD (Y, YH, LDYH, VSAV, SAVF, EWT, ACOR, IWM, WM, F, JAC,   &
      PSOL, NFLAG, RPAR, IPAR)
      ! write (*,*) "In DVSTEP: Back fr. VNLS"
!
      IF (NFLAG == 0) GOTO 450
!-----------------------------------------------------------------------
...
! Type declarations for local variables --------------------------------
!
      real(dp) AHATN0, ALPH0, CNQM1, CORTES, CSUM, ELP, EM, EM0, &
...
! Type declarations for local variables --------------------------------
!
      real(dp) ALPH0, ALPH1, HSUM, ONE, PROD, T1, XI, XIOLD, ZERO
...
!*DECK DVNLSD
      ...
      interface
        SUBROUTINE DVSOL (WM, IWM, X, IERSL)
          ...
          real(dp), dimension( : ), intent(inout) :: X, WM
          integer, dimension( : ), intent(inout) :: IWM
      ...
      interface
        ...
        IERPJ, RPAR, IPAR)
          use dvode_globals
          EXTERNAL F, JAC
          real(dp) YH, FTEM, SAVF, EWT
          INTEGER LDYH, IERPJ
          DIMENSION YH (LDYH, * ), FTEM ( * ), SAVF ( * ) , EWT ( * )
          real(dp), dimension( : ), intent(inout) :: Y, WM, RPAR
          integer, dimension( : ), intent( inout ) :: IWM, IPAR
...
! Type declarations for local variables --------------------------------
!
      real(dp) CCMAX, CRDOWN, CSCALE, DCON, DEL, DELP, ONE, RDIV,       &
      TWO, ZERO
...
!-----------------------------------------------------------------------
  220 M = 0
      DELP = ZERO
      ! write (*,*) "In DVNLSD: Calling DCOPY and F"
      CALL DCOPY (N, YH (1, 1), 1, Y, 1)
      CALL F (N, TN, Y, SAVF, RPAR, IPAR)
      ! write (*,*) "In DVNLSD: Ret. fr. DCOPY and F"
      NFE = NFE+1
      IF (IPUP <= 0) GOTO 250
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
  ...
  360 END DO
      ! write (*,*) "In DVNLS: Calling DVSOL"
      CALL DVSOL (WM, IWM, Y, IERSL)
      ! write (*,*) "In DVNLS: ret. fr. DVSOL"
      NNI = NNI + 1
...
!*DECK DVJAC
      ...
      interface
        SUBROUTINE DGBFA (ABD, LDA, N, ML, MU, IPVT, INFO)
          ...
          real(dp), dimension (LDA,N), intent (inout) :: ABD
...
! Type declarations for local variables --------------------------------
!
      real(dp) CON, DI, FAC, HRL1, ONE, PT1, R, R0, SRUR, THOU,  &
      YI, YJ, YJJ, ZERO
...
! End of setting JOK. --------------------------------------------------
!
      !   write (*,*) "In DVJAC", JOK, MITER, shape(WM)
...
! If JOK = -1 and MITER = 1, call JAC to evaluate Jacobian. ------------
         ...
         DO 110 I = 1, LENP
           WM (I + 2) = ZERO
  110    END DO
         ! write (*,*) "In DVJAC: Calling JAC"
         CALL JAC (N, TN, Y, 0, 0, WM (3), N, RPAR, IPAR)
         ! write (*,*) "In DVJAC: Ret. fr. JAC"
         IF (JSV == 1) CALL DCOPY (LENP, WM (3), 1, WM (LOCJS), 1)
      ENDIF
!
      ! write (*,*) "JOK and MITER are supposed to be -1 and 2 here,",JOK==-1, MITER==2
      IF ((JOK == -1).AND.(MITER == 2)) THEN
! If MITER = 2, make N calls to F to approximate the Jacobian. ---------
         NJE = NJE+1
         NSLJ = NST
         JCUR = 1
         ! write (*,*) "Calling DVNORM"
         FAC = DVNORM (N, SAVF, EWT)
         R0 = THOU * ABS (H) * UROUND * REAL (N, dp) * FAC
         IF (R0 == ZERO) R0 = ONE
         ! write (*,*) shape(WM)
         ! write (*,*) WM
         ! write (*,*) shape(WM)
...
! Multiply Jacobian by scalar, add identity, and do LU decomposition.
  ...
  580 END DO
      ...
      CALL DGBFA (WM(3:(2 + MEBAND * N )), MEBAND, N, ML, MU, IWM (31:(30 + N)), IER)
      IF (IER.NE.0) IERPJ = 1
      RETURN
...
!*DECK DVSOL
      ...
      interface
        ...
        use dvode_globals
          ...
          integer, dimension( : ), intent( inout ) :: IPVT
          real(dp), dimension(LDA,N), intent(inout) ::  ABD ! dimension used to be (LDA,*)
          real(dp), dimension( : ), intent(inout) :: B
          real(dp) DDOT ! intrinsic function, I think.
          real(dp) T
...
!
      ...
      real(dp) DI, HRL1, ONE, PHRL1, R, ZERO
!-----------------------------------------------------------------------
! The following Fortran-77 declaration is to cause the values of the
! listed (local) variables to be saved between calls to this integrator.
!-----------------------------------------------------------------------
...
!
  400 ML = IWM (1)
      MU = IWM (2)
      MEBAND = 2 * ML + MU + 1
      CALL DGBSL (WM(3:), MEBAND, N, ML, MU, IWM (31:(30+N)), X, 0)
      RETURN
...
!----------------------- END OF SUBROUTINE DEWSET ----------------------
      END SUBROUTINE DEWSET
!*DECK DVNORM
      real(kind(1.d0)) FUNCTION Old_DVNORM (N, V, W)
      use dvode_globals
!      INTEGER N, I
!      real(dp) SUM
!      real(dp), DIMENSION(N), intent(in) :: V, W
...
!***FIRST EXECUTABLE STATEMENT  DVNORM
      ...
      DO I = 1, N
       SUM = SUM + ( V (I) * W (I) )**2
      END DO
      Old_DVNORM = DSQRT (SUM / real(N,dp))
      RETURN
!----------------------- END OF FUNCTION DVNORM ------------------------
      END FUNCTION Old_DVNORM
!*DECK XERRWD
...
!  4. R1 and R2 are assumed to be in real(dp) and are printed
!     in D21.13 format.
!
...
!----------------------- End of Function IUMACH ------------------------
      END FUNCTION IUMACH
!*DECK DUMACH
      real(kind(1.d0)) FUNCTION DUMACH ()
        use dvode_globals
...
!***DESCRIPTION
!
!     DGEFA factors a real(dp) matrix by Gaussian elimination.
!
...
!***END PROLOGUE  DGEFA
      ...
      real(dp), dimension(LDA,N), intent( inout ) :: A ! A is sent as (:), but received here as (:,:)
!
      real(dp) T
      ...
      ! newshape(1) = LDA
      ! newshape(2) = N
      ! A = reshape(A,newshape) ! accepting A w/dimension(:) and reshaping to (:,:) didn't work
!
...
!***DESCRIPTION
!
!     DGESL solves the real(dp) system
...
!***DESCRIPTION
!
!     DGBFA factors a real(dp) band matrix by elimination.
!
...
!***DESCRIPTION
!
!     DGBSL solves the real(dp) band system
!     A * X = B  or  TRANS(A) * X = B
...
!***END PROLOGUE  DGBSL
      INTEGER LDA, N, ML, MU, JOB
      integer, dimension( : ), intent( inout ) :: IPVT
      real(dp), dimension(LDA,N), intent(inout) ::  ABD ! (LDA,*)
      real(dp), dimension( : ), intent(inout) :: B
      real(dp) DDOT ! intrinsic function, I think.
      real(dp) T
      INTEGER K, KB, L, LA, LB, LM, M, NM1
