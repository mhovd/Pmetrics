  ...
  360          CONTINUE
   ...
   10    CONTINUE
     ...
     2 , BAYPOS, NACTSUB )
        use npag_utils, only: pause, verifyval, maxnumeq,
...
2020   READ(21,2) READLINE
 ...
 4200   WRITE(*,1)
    1   FORMAT(//' THERE WAS AN ERROR IN THE READING OF PATIENT NOS.'/
     ...
     1   NADD)
        use npag_utils, only: pause, maxnumeq,max_RS_J,max_input_dim
        IMPLICIT REAL*8(A-H,O-Z)
...
123    FORMAT(/' YOUR PATIENT DATA FILES HAVE TOO MANY COLUMNS IN '/
  ...
  127    FORMAT(/' AT LEAST ONE OF YOUR PATIENT DATA FILES HAS TOO'/
   ...
   90    ISTART = J
    ...
    3   FORMAT(A1000)
     ...
     2   SUBMEAN,SUBLOGLIK,SUBSTD,SUBPERCOF,
     3   NAME,CHARTNO,SEX,NDD,NI,ASSAYC,IERRMOD)
        use npag_utils, only: pause, maxnumeq,max_pop_rand_varbs
        IMPLICIT REAL*8(A-H,O-Z)
  ...
  976    FORMAT(A976)
   ...
   31    FORMAT(/' THE MAXIMUM ENDING CYCLE NO IS LARGER THAN 999999,'/
     ...
     2   , k_prod_pr, k_sum_z_sq, i_skip_ig, i_do, i_cycle
...
!$omp Threadprivate(F)
        CALL FUNC(JSUB,IG
     1   ,NOBSER,F,NPX,PX,NBCOMP
     1   ,NDIM,MF,RTOL,ATOL
     3   ,TIMCOPY,SIGCOPY,YO,RSCOPY,BSCOPY
     2   ,INTLIST,IPAR,ObsError,RPAR,ERRFIL)
 ...
 10     continue
     ...
     1   NDIM,MF,RTOL,ATOL,TIMCOPY,SIGCOPY,YO,RSCOPY,BSCOPY,
     2   INTLIST,IPAR,ObsError,RPAR,ERRFIL)
       use npag_utils, only: useranal, pause, verifyval, shift
     1   , thesame, do140, SimConc
     2   , predlast3, maxnumeq, max_m_per_obs, max_SS_doses
     3   , max_ODE_params,max_doses,max_ODE_comps,max_RS_J
     4   , max_input_dim, k_gamma, k_flat, k_sfac, k_ofac
     5   , k_sum_z_sq, k_prod_pr, k_dvode_reserved, k_p_end
     6   , k_ig, k_jsub, i_ig, i_jsub, k_c0_base, k_c1_base
     7   , k_c2_base, k_c3_base, k_c4_base, k_c5_base
     8   , k_resolve, i_errmod, i_cycle
     9   , i_misval, i_skip_ig, i_is_log10, i_Nnormalobs
     &   , i_is_poisson_obs, i_Npoissonobs, i_dvode_reserved
      IMPLICIT none
...
! ADDED BSO(.,.) AS OF idm1x17.f.
       double precision, save, dimension(max_m_per_obs) :: TIMO
       double precision, save, dimension(max_doses) :: SIGO
       double precision, save, dimension(max_doses,max_RS_J) :: RSO
       double precision, save, dimension(max_doses,max_input_dim) :: BSO
! NEW PARALLEL CODE BELOW AS OF npageng28.f.
!$omp Threadprivate(BSO,RSO,SIGO,TIMO,X,Y)
...
!$omp Threadprivate(IKNS,KNS,KNT,KNSOLD)
       ...
       double precision, dimension(max_ODE_comps) :: XPRED, B
       ...
       integer WriteTestData
       integer UseInlineSimConc
      ...
      if (UseInlineSimConc .eq. 0) then
        ...
        Call SimConc(1,JSUB,IG                                          ! ijob, FirstPrivate, !$omp do loop counter
     1    , INTLIST(10),Y,INTLIST(9)                                    ! Saved Local threadPrivate
     2    , NBCOMP                                                      ! FirstPrivate
     3    , NPX,PX,NDIM,MF,RTOL,ATOL                                    ! NPX & PX private, others are FirstPrivate
     4    , TIMO,SIGO,RSO,BSO                                           ! local SAVED !$omp ThreadPrivate copies
     5    , INTLIST,IPAR,RPAR                                           ! !$omp ThreadPrivate
     6    , ERRFIL)
           if ((JSUB==3).and.(IG==2).and.(ipar(i_cycle)==7)) then       ! Choose these for Vori
           WriteTestData = 0
           if (WriteTestData == 1) then
             OPEN (21, FILE="SimConcTest01", STATUS="REPLACE"
     &        , FORM="UNFORMATTED", POSITION="REWIND", ACTION="WRITE")
             write (21) 1,JSUB,IG,INTLIST(10),Y,INTLIST(9),NBCOMP,NPX,PX
     1         ,NDIM,MF,RTOL,ATOL,TIMCOPY,SIGCOPY,RSCOPY,BSCOPY         ! *O are altered in SimConc
     2         ,INTLIST,IPAR,RPAR
             close(21)
             write (*,*) "Stopping program to write Julian's data"
             write (*,*) "ijob=", 1, "JSUB=", JSUB, "IG=", IG, "M=",
     1         M, "NUMEQT=",intlist(9),"NBCOMP=",NBCOMP,"NPX=",NPX,
     2         "NDIM=",NDIM,"MF=",MF,"RTOL=",RTOL,"SIZE(Y)=",size(Y),
     3         "size(TIM)=",size(TIMCOPY),"size(SIG)=",size(SIGCOPY),
     4         "size(RSCOPY)=",size(RSCOPY),"size(BSCOPY)=",size(BSCOPY)
             STOP                                                       ! the only way to preserve the new file
             WriteTestData = 2
           end if
           end if
      else
...
31      CONTINUE
        IF(ID .NE. 1) then
          ...
          DO 2010 I=1,INTLIST(9)
            Y(KNTM1,I)=YT(I)
2010      continue
! 20200713 Matches UseInlineSimConc=0
!        write (*,*) JSUB,IG,KNT,KNS,"YPRED(KNT,KNS)="
!     1  ,(YT(I),I=1,INTLIST(9))
        endif
55      IF (ID.NE.0) then
  35    CONTINUE
...
! 20200714 --  UseInlineDO140 \in (0,1) identical output
!       DO 140 I=1,NOBSER
!         DO 140 J=1,NUMEQT
!        write (*,*) IPAR(i_cycle),UseInlineDO140,JSUB,IG
!     2   ,((ObsError(I,J),I=1,INTLIST(10)),J=1,INTLIST(9))
! 20200715 -- UseInline=1 -> Pr(5|G)=0, but UseInline=0 has no problems
!        write (*,*) IPAR(i_cycle), JSUB, IG, RPAR(k_sum_z_sq)
!     1   , RPAR(k_sfac), RPAR(k_ofac), RPAR(k_prod_pr)
! The write stmnts for UseInline* = 0 or 1 seem to be identical; However,
! 19761 pts are skipped of the 20k that are thrown!? for UseInline=1
! So, I suspect that the issue is downstream from here, in main.
! FIXED problem was the "else" in if PoissonProbability > 0.00000001
! in main.
! NEW PARALLEL CODE BELOW AS OF npageng28.f
!	 ND = NDO
!	 DO I=1,INTLIST(8)
...
!	 END DO
      RETURN
      END
      SUBROUTINE Old_USERANAL(JSUB,IG,X,TIN,TOUT,
...
! NEW PARALLEL CODE BELOW AS OF npageng28.f.
        ...
        if (UseInlineSimConc .eq. 1) then
          CALL FUNC2(JSUB,IG
     1   , NOBSER, YPRED, NUMEQT
     2   , NBCOMP
     3   , NPP,ESTML,NDIM,MF,RTOL,ATOL
     4   , TIMCOPY,SIGCOPY,RSCOPY,BSCOPY
     5   , ObsError,INTLIST,IPAR,RPAR,ERRFIL)
        else
        CALL SimConc(2,JSUB,IG
     1   , NOBSER, YPRED, NUMEQT
     2   , NBCOMP
     3   , NPP,ESTML,NDIM,MF,RTOL,ATOL
     4   , TIMCOPY,SIGCOPY,RSCOPY,BSCOPY
     5   , INTLIST,IPAR,RPAR,ERRFIL)
        end if
        RETURN
        END
        SUBROUTINE FUNC2(JSUB,IG
     1   , M, YPRED, NUMEQT
     2   , NBCOMP
     3   , NPP,ESTML,NDIM,MF,RTOL,ATOL
     4   , TIM,SIG,RS,BS
     ...
     1   , verifyval, shift, thesame, predlast3
     2   , maxnumeq, max_m_per_obs, max_SS_doses
     3   , max_ODE_params, max_doses, max_ODE_comps, max_RS_J
     4   , max_input_dim, k_dvode_reserved, k_p_end, k_jsub, k_ig
     5   , i_do, i_ig, i_jsub, i_dvode_reserved
       implicit none
! NEW PARALLEL CODE BELOW AS OF npageng28.f.
      integer N,ND,NI,NP
     ...
     1  :: XSTORE
      ...
      double precision, save, dimension(max_m_per_obs,MAXNUMEQ) :: Y
!$ThreadPrivate(T,TOUT,X,XPRED,XVERIFY,XSTORE,SIGO,RSO,TLAG,FA,YT,Y)
      double precision DOSEINT
      integer NTL, KNT, KNSNEW, KNS, J
      integer ISTEADY, ISKIPBOL, III
      integer IKNS, ID, I
      integer ICONV,ISAME
      integer KNTM1,NDO,NN,NSET
       N = NDIM
       ND = intlist(8)
       NI = intlist(7)
       NP = NPP
...
40      IF(KNT .LE. M) GO TO 45
      ...
      RETURN
      END
        SUBROUTINE Old_SimConc(ijob,JSUB,IG
     1   , M, YPRED, NUMEQT
     2   , NBCOMP
     3   , NPP,ESTML,NDIM,MF,RTOL,ATOL
     4   , TIM,SIG,RS,BS
     ...
     &   , i_do, i_ig, i_jsub, i_dvode_reserved
      IMPLICIT NONE
      integer, intent(in) ::  ijob, JSUB, IG, M
      double precision, dimension(max_m_per_obs,NUMEQT) :: YPRED
      integer, intent(in) ::  NUMEQT, NPP
      double precision, dimension(max_ODE_params), intent(in) :: ESTML
      integer, intent(in) ::  NDIM,MF
      integer, dimension(max_input_dim), intent(in) :: NBCOMP
      double precision, intent(in) :: RTOL
      double precision, dimension(max_ODE_comps)
     1 , intent(in)  :: ATOL
      double precision, dimension(max_m_per_obs)
     1 , intent(inout) :: TIM
      double precision, dimension(max_doses)
     1 , intent(inout) :: SIG
      double precision, dimension(max_doses,max_RS_J)
     1 , intent(inout) :: RS
      double precision, dimension(max_doses,max_input_dim)
     1 , intent(inout) :: BS
      double precision, dimension(max_m_per_obs,MAXNUMEQ)
     1 , intent(inout) :: ObsError
      integer, dimension(128),intent(inout) :: INTLIST
      ...
      integer NI,NP
      integer NOS
      integer NTL, NDO, KNT, KNS, J
      integer ISTEADY, ISKIPBOL, III
      integer IKNS, ID, I, KNSOLD
      double precision, dimension(max_doses) :: SIGO
      double precision, dimension(max_doses,max_RS_J) :: RSO
      double precision, dimension(max_m_per_obs,MAXNUMEQ) :: Y
      integer, save :: KNTM1,NN,NSET,ICONV,ND,NDRUG,NADD,N,ISAME
      double precision, save :: T, TOUT, DOSEINT
      double precision, save, dimension(max_RS_J) :: R
      double precision, save, dimension(max_ODE_comps) :: B
      double precision, save, dimension(max_ODE_comps) :: X
      double precision, save
     1  , dimension(max_SS_doses,max_ODE_comps) :: XSTORE
      double precision, save, dimension(max_ODE_comps) :: XPRED
      double precision, save, dimension(max_input_dim) :: TLAG, FA
      double precision, save, dimension(MAXNUMEQ) :: YT
      double precision, save, dimension(100) :: XVERIFY
!$omp ThreadPrivate(KNTM1,NN,NSET,ICONV,ND,NDRUG,NADD,N,ISAME)
...
!$omp ThreadPrivate(XVERIFY)
      ...
      DO I=1,ND
        ...
        END DO
      END DO
      IF(N .NE. 0) CALL GETIX(N,X,ESTML,R,B,INTLIST)
...
83      IF(NDRUG .ne. 0 .AND. N .ne. 0) then
          IF(ISKIPBOL .EQ. 0) THEN
            DO I=1,NDRUG
              X(NBCOMP(I))=X(NBCOMP(I))+BS(KNS-1,I)*FA(I)
            END DO
...
! NEW PARALLEL CODE BELOW AS OF npageng28.f.
        ...
        if (UseInlineSimConc.eq.1) then
          CALL FUNC3(JSUB,IG
     1    , NUMT,YYPRED,TPRED,NUMEQT
     2    , NBCOMP
     3    , NPP,ESTML,NDIM,MF,RTOL,ATOL
     4    , TIMCOPY,SIGCOPY,RSCOPY,BSCOPY
     5    , INTLIST,IPAR,ObsError,RPAR,ERRFIL)
        else
          Call SimConc(3,JSUB,IG
     1    , NUMT,YYPRED,INTLIST(9)
     2    , NBCOMP
     3    , NPP,ESTML,NDIM,MF,RTOL,ATOL
     4    , TPRED,SIGCOPY,RSCOPY,BSCOPY
     5    , INTLIST,IPAR,RPAR,ERRFIL)
        end if
        RETURN
        END
     ...
     2      TIM,SIG,RS,BS,INTLIST,IPAR,ObsError,RPAR,ERRFIL)
         use npag_utils, only : useranal, pause
     1    , shift, thesame, predlast3
     2    ,maxnumeq,max_m_per_obs,max_RS_J
     3    ,verifyval, max_SS_doses
     4    ,max_ODE_params, max_doses, max_ODE_comps, max_input_dim
     5    ,k_dvode_reserved, k_ig, k_jsub, k_p_end, i_jsub, i_ig
     6    ,i_dvode_reserved
      implicit none
...
40    IF(KNT  .LE. NUMT) GO TO 45
     ...
     2  DENSTOR,EXX,CORDEN,CORHOLD,YPREDPOP,YPREDPOPT,YPREDBAY,CORDLAST)
        USE OMP_LIB
        USE npag_utils, only: pause,makevec,check_input_array_size
     1   ,cp_theta_to_rpar,expand_grid
     2   ,verifyval,cp_lrcs_to_rpar,max_input_dim,maxnumeq,max_m_per_obs
     3   ,max_ODE_params,max_pop_rand_varbs,max_doses,max_ODE_comps
     4   ,max_pop_varbs,max_pop_params,max_RS_J,k_gamma,k_flat
     5   ,k_sfac,k_ofac,k_sum_z_sq,k_prod_pr,i_skip_ig,i_do,i_cycle
     6   ,i_errmod,i_is_poisson_obs,i_is_log10,i_Npoissonobs,i_Jsub,i_IG
     7   ,k_resolve, print_matrix, obs_is_missing, obs_is_BLQ
      IMPLICIT REAL*8(A-H,O-Z)
      interface
     ...
     &                 isupres)
      ...
      integer ldpsi,ldtheta,npoint,nsub,ijob,nvar,IHESS,keep
      double precision :: fobj, gap
      double precision, dimension(:) :: x(:),dx(:),y(:),dy(:)
     ...
     8  CORDLAST(MAXGRD,MAXDIM+1),XVERIFY(100)
      DIMENSION SIG(max_m_per_obs,MAXNUMEQ),
     ...
     4 NUMT(MAXSUB),
     5 TTPRED(MAXSUB,7200),TEND(99),NOMAXTIM(MAXSUB),
     6 TENDSUB(MAXSUB,99),
     ...
     1 max_pop_rand_varbs) :: COV, CORR, E
      ...
      double precision, save, dimension(max_ODE_params) :: PX
       CHARACTER PAR(max_pop_rand_varbs)*11
       CHARACTER PARRANFIX(max_pop_varbs)*11
       CHARACTER PARFIX(max_pop_params)*11
      integer III, JJJ, JJ, IE, IOLD
      integer ierrmod, ICYCLE
...
!       COMMON/TOCALC/gamma,flat,AB,PX,IRAN,NOFIX,NSUB
      logical input_arrays_within_bounds
       integer nnvar, nkeep, nmaxsub, nmaxgrd, nnactve, nnsub, nijob
       double precision fobj_in
       double precision, dimension(MAXSUB,MAXACT) :: PYJGX_IN
       integer write_J_data,nrow,ncol
       integer NVAR,NOFIX,NRANFIX
       integer, dimension(max_ODE_params) :: IRAN
       integer ijob
       double precision, dimension(MAXGRD) :: ipmlambda, ipmtheta
     &    ,ipmdeltalambda, ipmdeltatheta
       integer NInDO1000
       real PInDO1000
       double precision, dimension(max_m_per_obs,NUMEQT) :: YPRED
       integer UseInlineDO140
...
!      SLPYJ=0.D0
     ...
     1    , "of", NSUB, "in cycle", ICYCLE, "with res", RPAR(k_resolve)
        write (*,*)
...
!$omp&FirstPrivate(NVAR,NRANFIX,IRANCOPY,NOFIXCOPY,NBCOMP,ierrmod)
!$omp&Private(PX,NPX)
!$omp Do
...
!$omp critical
           ...
           if (NNInDO800.eq.2048) then
             PInDO800 = 100.0*REAL(NInDO800)
     1        /(REAL(IterLast) - REAL(IterFirst))
        write(*,*) PInDO1000, "%     --- DO 800 ------- Obs:", JSUB,
     1    "Support:", IG, "of", IterFirst, "to", IterLast, PInDO800,"%"
          NNInDO800 = 0
        end if
!$omp end critical
     ...
     1    RANFIXEST, PX)
        call cp_theta_to_rpar(JSUB,IG,PX,rpar)
     ...
     1        * DEXP(-.5D0*RPAR(k_sum_z_sq))/RPAR(k_sfac)/RPAR(k_ofac)
          if ((IG.eq.1).and.(IPAR(i_cycle).eq.1)) then
           write (*,9783) "PYJGX =",PYJGX(JSUB,IG)
     1        , 10**RPAR(k_prod_pr), DEXP(-.5D0*RPAR(k_sum_z_sq))
     2        , RPAR(k_sfac), RPAR(k_ofac)
          endif
 9783      format(A,D17.10,D17.10,D17.10,D17.10,D17.10)
           ENDIF
...
!       SLPYJ=SLPYJ+DLOG(PYJ)
 ...
 1243 FORMAT(/' The Adaptive Grid optimization process could take '/
     ...
     1 write(6,*) 'condensing current ', nactve, ' point grid'
      ...
      write_J_data = 0
      if (write_J_data .eq. 1) then
        if (IPAR(i_cycle) .eq. 2) then
          do iii = 1,maxact
            do jjj = 1,nvar+1
              pyjgx_in(iii,jjj) = pyjgx(iii,jjj)
            end do
          end do
        end if
      end if
      DO III = 1,maxgrd
         ipmdeltalambda(III) = denstor(III,1)
         ipmtheta(III) = denstor(III,2)
         ipmdeltatheta(III) = denstor(III,3)
         ipmlambda(III) = corden(III,nvar+1)
      END DO
      call emint(pyjgx,maxsub,corden,maxgrd,nactve,nsub,ijob
     &,ipmlambda,ipmdeltalambda,ipmtheta,ipmdeltatheta
     &,fobj,gap,nvar,keep,IHESS,errfil,ISUPRES)
      write (*,*) "I'm back from emint w/ijob=",ijob
      DO III = 1,maxgrd
         denstor(III,1) = ipmdeltalambda(III)
         denstor(III,2) = ipmtheta(III)
         denstor(III,3) = ipmdeltatheta(III)
         corden(III,nvar+1) = ipmlambda(III)
      END DO
      if (write_J_data .eq. 1) then
        write (*,*) "Check"
        write (*,*) "pyjgx", pyjgx(1,1),pyjgx(1,5),pyjgx(1,9)
     &   , pyjgx(7,9),pyjgx(8,4),pyjgx(3,1),pyjgx(5,5)
        if (IPAR(i_cycle) .eq. 2) then
          OPEN (21, FILE="ipmtest01", STATUS="REPLACE"
     &       , FORM="UNFORMATTED", POSITION="REWIND", ACTION="WRITE")
          write (*,*) shape(pyjgx_in)
          write (21) maxsub,maxgrd,pyjgx_in,maxsub
     &   , corden,maxgrd
     &   , nactve,nsub,ijob
     &   , corden(1:maxgrd,nvar+1),denstor,fobj,gap,nvar,keep
     &   , IHESS,errfil,ISUPRES
          CLOSE(21)
          write (*,*) "Julian's data before emint()"
          write (*,*) corden(1:20,nvar+1)
          write (*,*) "pyjgx_in", pyjgx_in(1,1),pyjgx_in(1,5)
     &   , pyjgx_in(1,9), pyjgx_in(7,9),pyjgx_in(8,4),pyjgx_in(3,1)
     &   , pyjgx_in(5,5)
          write (*,*) fobj, gap, nvar, keep
        OPEN (21, FILE="ipmtest01", STATUS="OLD", FORM="UNFORMATTED",
     &       POSITION="REWIND", ACTION="READ")
        read (21) nrow,ncol,pyjgx,nmaxsub,corden,nmaxgrd,nnactve
     &   , nnsub,nijob
     &   , corden(1:maxgrd,nvar+1),denstor,fobj_in,gap,nnvar,nkeep
     &   , IHESS,errfil,ISUPRES
        CLOSE(21)
        write (*,*) "Julian's read data"
        write (*,*) corden(1:20,nvar+1)
        write (*,*) nrow, ncol, shape(pyjgx)
        write (*,*) "pyjgx",nrow,ncol,pyjgx(1,1),pyjgx(1,5),pyjgx(1,9)
     &   , pyjgx(7,9),pyjgx(8,4),pyjgx(3,1),pyjgx(5,5)
        write (*,*) "fobj", fobj, gap, nnvar, nkeep
        if (fobj.ne.fobj_in) write (*,*) "w/r precision err"
      endif
      end if
...
14001 continue
      IF(ISUPRES .EQ. 0) write(*,*) 'fobj in',fobjminu,fobjbase,fobjplus
      IF(ISUPRES .EQ. 0) write(6,*) 'fobjbest=',fobj
      write(*,*) 'cycle=',ipar(i_cycle),'fobj in',fobjminu
     1   ,fobjbase,fobjplus,fobj1,fobjbest
      fact=ngrid/volspa
      do i=1,nactve
 ...
 2100   CONTINUE
        ...
        END DO
        JJ=J
        CALL NOTINT(VOLSPA,NGRID,NACTVE,WORK,MAXGRD,E(II,JJ))
 1100   continue
 1101   continue
        write (*,*) "Passed 1100"
          DO I=1,NVAR
 ...
 5103   FORMAT(1X,30(G12.6,1X))
   ...
   13 FORMAT(/' THE COV MATRIX IS, IN LOWER TRI FORM: ')
      ...
      IF(ISUPRES .EQ. 0) WRITE(*,5103) (XVERIFY(J),J=1,I)
  200 continue
      DO I = 1,NVAR
       STD(I)=DSQRT(COV(I,I))
 ...
 5456    FORMAT(/' WHICH MEANS THAT GAMMA WAS NOT ESTIMATED IN THIS RUN.
     1 ')
      write (*,*) "IF(NRANFIX.GT.0.AND.ICYCLE.EQ.1)",NRANFIX,ICYCLE
     ...
     3  IPAR,ObsError,RPAR,gamma,flat,AB,PX,IRAN,NVAR,NOFIX,NRANFIX,
     4  NSUB,ierrmod,errfilname)
 ...
 1261    FORMAT(/' THE USER CHOSE TO STOP THE PROGRAM AT CYCLE NO. '/
     1' ',I7,' ... THE ORIGINAL NO. OF MAXIMUM CYCLES WAS ',I7//)
      ...
      ximprove=fobj-prefobj
       ...
       XVERIFY(7) = fobjplus
       XVERIFY(8) = fobjminu
       CALL VERIFYVAL(8,XVERIFY)
       write(91,9191) icycle,(XVERIFY(IXV),IXV=1,8),
     &nactve1,nactve0
 9191 format(i5, 2f15.6,5x,f6.5,5x,5f15.6,5x,2i5)
      IF(ISUPRES .EQ. 0)
     ...
     1 write(6,*) 'current flat weight = ',flat*gamma
      ...
      write(*,*) 'res set to .2 ',' checkbig=',checkbig
 ...
 1024    FORMAT(1X,G14.4,' <-- CONVERGENCE OCCURS WHEN THIS NO. < ',F20.
     ...
     1    resolve, corden, ab)
      ...
      isupres = III
        prefobj=fobj
       write (*,*) "fobj, gamma, res ="
     1   , fobj, gamma, RPAR(k_resolve)
        GO TO 1001
  900 continue
      IF(NRANFIX .GT. 0) THEN
       DO I = 1,NRANFIX
 ...
 8506   FORMAT(////' THE FOLLOWING RESULTS ARE FOR SUBJECT ',I4)
     ...
     2     INTLIST,ERRFILNAME)
        ...
        DO 241 I=1,NOBSER
          DO 240 J=1,NUMEQT
            Y = YO(I,J)
            IF(Y .EQ. obs_is_BLQ) THEN
              write (*,*) JSUB,"is BLQ for (obs, eqn):",I,J
              Y = 0.d0
              GO TO 240
            ENDIF
            IF(Y .EQ. obs_is_missing) THEN
              MISVAL = MISVAL+1
              GO TO 240
            ENDIF
            if (C4(J).eq.10) IPAR(i_is_log10+J) = 10
            if (C5(J).eq.229) then
              ...
              if(ierrmod.eq.4) sig(i,j) = gamma*flat
              ObsError(I,J) = sig(I,J)
            IF(SIG(I,J) .LE. 0) THEN
              write (*,*) "Norm ERR:", SIG(I,J), C0(JJ),C1(JJ),C2(JJ)
     1         , C3(JJ), C4(JJ), C5(JJ), Y, ierrmod, gamma
            end if
 ...
 1181   FORMAT(/'***************** END OF THE npagdriv.f FILE **********
     ...
     1    TIM,SIG,RS,BS,INTLIST,ERRFIL)
         use npag_utils, only: pause,maxnumeq,max_m_per_obs
...
! wmy20190628 --
     ...
     1    RS(max_doses,max_RS_J), BS(max_doses,max_input_dim)
        character ERRFIL*20
  ...
  126    FORMAT(/' AT LEAST ONE OF YOUR PATIENT DATA FILES HAS TOO'/
   ...
   50   READ(27,1) READLINE
        ...
        USE npag_utils, only: pause,max_pop_rand_varbs
        IMPLICIT REAL*8(A-H,O-Z)
  ...
  100   continue
     ...
     1  PROB(1998),CUMPRO(0:1998)
        integer IND025, IND25, IND50, IND75, IND975
        IND025 = 0
        IND25 = 0
        IND50 = 0
        IND75 = 0
    ...
    5   continue
        ...
        END DO
       CALL NOTINT(VOLSPA,NGRID,NACTVE,PROD,MAXGRD,PROB(INTR))
 1000  continue
        INTMAX=1
        DO INTR=2,NEWINT
 ...
 1200 CONTINUE
     ...
     1  ,cp_theta_to_rpar,pause,max_ODE_params,max_pop_rand_varbs
     2  ,max_doses,max_pop_params
     3  ,max_ODE_comps,max_pop_varbs,max_RS_J,max_input_dim
     4  ,k_sfac,k_ofac,k_sum_z_sq,k_prod_pr,i_skip_ig,i_do
      IMPLICIT REAL*8(A-H,O-Z)
...
!  $omp ThreadPrivate(/PARAMD/)
 ...
 1241   FORMAT(//' THE BAYESIAN POSTERIOR DENSITY RESULTS FOLLOW FOR '/
     ...
     1        * DEXP(-.5D0*RPAR(k_sum_z_sq))/RPAR(k_sfac)/RPAR(k_ofac)
            ...
            write (*,*) JSUB,IG,"sum z^2 > 22708",RPAR(k_sum_z_sq)
          ENDIF
            PYJGXX(IG) = PYJGX
 ...
 2031    FORMAT(///' FOR SUBJECT NO. ',I4,' THE MAXIMUM NO. OF '/
  ...
  976    FORMAT(A976)
   ...
   10   RETURN
     ...
     1   pause,maxnumeq,max_m_per_obs,max_doses,
     2   max_RS_J
      IMPLICIT REAL*8(A-H,O-Z)
...
1920   BACKSPACE(23)
 ...
 4031    FORMAT(/' IN SUBROUTINE NEWWORK1, TWO CONSECUTIVE DOSE TIMES'/
  ...
  150 END DO
     ...
     3    gamma,flat,AB,PX,IRAN,NVAR,NOFIX,NRANFIX,NSUB,
     4    ierrmod,errfilname)
        use npag_utils, only: maxnumeq,max_m_per_obs,max_ODE_params
     1    ,cp_lrcs_to_rpar
     ...
     2 C0(NUMEQT),C1(NUMEQT),C2(NUMEQT),C3(NUMEQT),C4(NUMEQT),C5(NUMEQT)
      ...
      double precision, dimension(max_ODE_params) :: PX
      integer, dimension(max_ODE_params) :: IRAN
      integer NVAR,NOFIX,NRANFIX,NSUB
       integer ierrmod
       character*20 errfilname
       integer  NDIM,MF
       real*8 RTOL
  ...
  141   continue
     ...
     3    INTLIST,IPAR,ObsError,RPAR,gamma,flat,AB,PX,IRAN,NVAR,NOFIX,
     4    NRANFIX,NSUB,ierrmod,errfilname)
      use npag_utils, only: maxnumeq,max_m_per_obs,max_ODE_params
...
1       continue
     ...
     4    gamma,flat,AB,PX,IRAN,NVAR,NOFIX,NRANFIX,NSUB,
     5    ierrmod,errfilname)
        Y(NN)=FN
        ICOUNT=ICOUNT+1
