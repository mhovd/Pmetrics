	SUBROUTINE IDPC(JSUB,IG,X,SUMSQJ,INTLIST,RPAR,IPAR)
        use npag_utils, only : i_mf, i_ndim, i_nparams
	...
	CALL SYMBOL(NBCOMP)
      ...
      SUBROUTINE FUNC(JSUB,IG,M,SUMSQJ,INTLIST,RPAR,IPAR)
       use npag_utils, only: thesame, shift, pause, SimConc
     1  , maxnumeq, max_m_per_obs, max_ODE_params, max_RS_J
     2  , max_ODE_comps, practice_c_call
     3  , k_dvode_reserved, k_rtol, k_atol, k_p_end, k_jsub, k_ig
     4  , i_dvode_reserved, i_jsub, i_ig, i_mf, i_ndim, i_nparams
       implicit none
      COMMON/PARAMD/ P
      COMMON/STATE/ X
      COMMON/BOLUSCOMP/NBCOMP
      ...
      integer NPL,NOS,NDRUG,NADD
      double precision, dimension(594) :: TIM
      double precision, dimension(5000) :: SIG, SIGO
      double precision, dimension (5000,max_RS_J) :: RS, RSO
      double precision, dimension(max_m_per_obs,maxnumeq) :: Y,YO,STDEV
      double precision, dimension(5000,7) :: BS
      double precision, dimension (max_RS_J) :: R
      double precision, dimension(20) :: B
      CHARACTER ERRFIL*20
      integer JSUB,IG,M
      double precision, dimension(maxnumeq) :: SUMSQJ
      integer, dimension(128) :: INTLIST
      double precision, dimension(257) :: RPAR
      integer, dimension(257) :: IPAR
      ...
      integer knt, kns, kntm1, knsnew, isteady, iskipbol, isame
      integer ikns, id, iconv, i
      integer III,JSUBmain,J,NDO
      double precision, dimension(7) :: FA,TLAG
      double precision, dimension(100) :: XVERIFY
      double precision, dimension(100,20) :: XSTORE
      double precision, dimension(20) :: XPRED
      double precision, dimension(maxnumeq) :: YT
       ...
       mf = ipar(i_mf)
       rtol = rpar(k_rtol)
       do i=1,max_ODE_comps
         atol(i) = rpar(k_rtol + i)
       end do
        JSUBmain = JSUB
...
40      IF(KNT .LE. M) GO TO 45
        ...
        CALL SimConc(ijob,JSUBmain,IG
     2    , M, Y, NUMEQT
     3    , NBCOMP
     4    , NPP,ESTML,NDIM,MF,RTOL,ATOL
     5    , TIM,SIG,RS,BS
     6    , INTLIST,IPAR,RPAR,ERRFIL)
        call practice_c_call(NPP,ESTML)
      end if
        DO J=1,NOS
         SUMSQJ(J) = 0.D0
         DO I=1,M
	 ...
	 END DO
      ...
      END
        ...
        use npag_utils, only : i_mf, i_nparams, i_ndim
        IMPLICIT REAL*8(A-H,O-Z)
        PARAMETER(MAXNUMEQ=7)
        COMMON/SUM2/ M,NPNL
...
40      IF(KNT .LE. M) GO TO 45
	...
	SUBROUTINE IT2B(MAXSUB,MAXDIM,MAXGRD,NUMEQT,MAXOBDIM,PAREST,
     ...
     2  VEC)
        use npag_utils, only : pause, makevec, k_rtol, i_mf
        IMPLICIT REAL*8(A-H,O-Z)
        PARAMETER(MAXNUMEQ=7)
...
2346   FORMAT(//' A S.D. < 0 FOR JSUB = ',I5,'. RERUN THE '/
  ...
  140 CONTINUE
        ...
        ipar(i_nparams) = NVAR + NOFIX + NRANFIX
...
4324   FORMAT(/' THE AVERAGE LOG-LIKELIHOOD OF THE PATIENT DATA FILES'/
 ...
 9016 FORMAT(//' AND THE REVISED COV. MATRIX FOR THE ',I2,' RANDOM'/
     1' PARAMETERS IS: ')
       ...
       END DO
      ENDIF
      write (*,*) "Checking stop condition: ISTOP =", iter, maxit, istop
	IF(ITER .EQ. MAXIT .OR. ISTOP .EQ. 1) THEN
	REWIND(27)
	DO 6000 JSUB=1,NSUB
          IG=JSUB
...
1001    DO 1 I=1,N
1       P(I,NN)=START(I)
        CALL FUNC(N,START,FN,JSUB,IG,INTLIST,IPAR,RPAR)
        Y(NN)=FN
        ICOUNT=ICOUNT+1
...
8       PSTAR(I)=(1.0D0+RCOEFF)*PBAR(I)-RCOEFF*P(I,IHI)
        CALL FUNC(N,PSTAR,FN,JSUB,IG,INTLIST,IPAR,RPAR)
        YSTAR=FN
        ICOUNT=ICOUNT+1
...
9       P2STAR(I)=ECOEFF*PSTAR(I)+(1.0D0-ECOEFF)*PBAR(I)
        CALL FUNC(N,P2STAR,FN,JSUB,IG,INTLIST,IPAR,RPAR)
        Y2STAR=FN
        ICOUNT=ICOUNT+1
...
17      XMIN(I)=P(I,J)
        CALL FUNC(N,XMIN,FN,JSUB,IG,INTLIST,IPAR,RPAR)
        Y(J)=FN
18      CONTINUE
        ICOUNT=ICOUNT+NN
...
26      CONTINUE
        RETURN
        END
	SUBROUTINE MAPBAYS(NVAR,VEC,FNTVAL,JSUB,IG,INTLIST,IPAR,RPAR)
        use npag_utils, only : makevec, i_mf, i_ndim, i_nparams
	IMPLICIT REAL*8(A-H,O-Z)
      PARAMETER(MAXNUMEQ=7)
 ...
 4200   WRITE(*,1)
  ...
  976    FORMAT(A976)
   ...
   10   RETURN
      ...
      use npag_utils, only : pause, thesame
      IMPLICIT REAL*8(A-H,O-Z)
      PARAMETER(MAXNUMEQ=7)
 ...
 4031    FORMAT(/' IN SUBROUTINE NEWWORK1, TWO CONSECUTIVE DOSE TIMES'/
  ...
  150 END DO
	...
	SUBROUTINE CALCRF(NTOTPAR,VEC,FNTVAL,NUMEQT,YO,C0,C1,C2,C3,
     1  C4,C5,GAMMA,JSUB,IG,INTLIST,IPAR,RPAR)
	IMPLICIT REAL*8(A-H,O-Z)
      PARAMETER(MAXNUMEQ=7)
...
8       PSTAR(I)=(1.0D0+RCOEFF)*PBAR(I)-RCOEFF*P(I,IHI)
     ...
     1    GAMMA,JSUB,IG,INTLIST,IPAR,RPAR)
        YSTAR=FN
        ICOUNT=ICOUNT+1
        IF(YSTAR.GE.YLO) GO TO 12
        IF(ICOUNT.GE.KCOUNT) GO TO 19
